	  ...
	  `(("{{TITLE}}" . ,title)
	    ...
	    ("{{LAST_MICROBLOG_DESCRIPTION}}" . ,(when (string= slug "/")
						   (let* ((micro-years (nori-site-get-microblog-years index))
						     ...
						     desc)))
	    ("{{RECENT_POSTS}}" . ,(when (string= slug "/") (nori-site-render-list "summary-with-image-item.html" (take nori-site-featured-post-number (alist-get 'list-items (cdr (assoc "post/" index)))) nil )))
...
<item>
  ...
  <enclosure url="{{ITEM_FEATURED_IMAGE}}" type="image/jpg"></enclosure>
  <link>{{ITEM_URL}}</link>
  <pubDate>{{ITEM_DATE_HUMAN}}</pubDate>
  <author>nori@noriparelius.com (Nori Parelius)</author>
  <guid>{{ITEM_URL}}</guid>
  <description>
    <![CDATA[
	     <img src={{ITEM_FEATURED_IMAGE}} width="800" height="150" />
	     {{ITEM_CONTENTS}}
    ]]></description>
</item>
#+end_src

  ...
  (let* ((layout (alist-get 'layout metadata))
	  ...
	  `(("{{TITLE}}" . ,title)
	    ...
	    ("{{LAST_MICROBLOG_DESCRIPTION}}" . ,(when (string= slug "/")
						   (let* ((micro-years (nori-site-get-microblog-years index))
						     ...
						     desc)))
	    ("{{RECENT_POSTS}}" . ,(when (string= slug "/") (nori-site-render-list "summary-with-image-item.html" (take nori-site-featured-post-number (alist-get 'list-items (cdr (assoc "post/" index)))) org-buffer nil )))
	...
	(cond
	 ...
	 ((string= layout "microblog-section.html")
	  ...
	  (setq microblogp t)))
	(setq replacements (append replacements `(("{{LIST}}" . ,(nori-site-render-list listing-template-name list-items org-buffer microblogp)))))
...
(defun nori-site-render-list (listing-template-name list-items org-buffer get-html)
  (let* ((listing-template (nori-site-load-template listing-template-name))
	  ...
	  (dolist (item-slug list-items)
	    (let* ((item (assoc item-slug index))
		   (item-html (if get-html (nori-site-export-subtree-to-html org-buffer (alist-get 'begin (cdr item)))
				""))
		   ...
		   (item-replacements
		    `(("{{ITEM_URL}}" . ,(concat "/" (alist-get 'slug (cdr item))))
		      ...
		      ("{{ITEM_DATE_RFC822}}" . ,(when item-date (format-time-string "%a, %d %b %Y %H:%M:%S GMT" (date-to-time item-date))))
...
(defun nori-site-generate-rss (index)
  (let* ((file-name "index.xml")
	 ...
	 (replacements
	  `(("{{LAST_BUILD_DATE}}" . ,(format-time-string "%a, %d %b %Y %H:%M:%S GMT" (current-time)))
	    ("{{RSS_LIST}}" . ,(nori-site-render-list item-layout list-items (find-file-noselect nori-site-source-org-filename) t))
	    ))
	 (page (nori-site-render-template template replacements)))
    (with-temp-file file-path-file
      (insert page))
...
<item>
  ...
  <enclosure url="{{ITEM_FULL_FEATURED_IMAGE}}" type="image/{{IMAGE_EXT}}"></enclosure>

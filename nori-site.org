...
(defun nori-site-render-all-pages ()
  ...
  (let* ((publish-dir (expand-file-name nori-site-publish-directory))
    ...
    (nori-site-generate-404 "404" publish-dir)
    ))
  ...
  (let* ((layout (alist-get 'layout metadata))
	  ...
	  `(("{{TITLE}}" . ,title)
	    ...
	    ("{{LAST_MICROBLOG_DESCRIPTION}}" . ,(when (string= slug "/")
						   (let* ((micro-years (nori-site-get-microblog-years index))
						     ...
						     desc)))
	    ("{{RECENT_POSTS}}" . ,(when (string= slug "/") (nori-site-render-list "summary-with-image-item" (take nori-site-featured-post-number (alist-get 'list-items (cdr (assoc "post/" index)))) nil )))
	...
	(cond
	 ...
	 ((string= layout "microblog-section")
	  ...
	  (setq microblogp t)))
	(setq replacements (append replacements `(("{{LIST}}" . ,(nori-site-render-list listing-template-name list-items microblogp)))))
...
(defun nori-site-render-list (listing-template-name list-items microblogp)

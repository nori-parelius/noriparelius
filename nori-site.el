...
(defun nori-site--render-all-pages (org-buffer archive-html tag-cloud-html)
  ...
  (let* ((file-name "index.xml")
	 ...
	 (replacements
	  `(("{{LAST_BUILD_DATE}}" . ,(format-time-string "%a, %d %b %Y %H:%M:%S %Z" (apply #'encode-time (decode-time (current-time))) "GMT"))
	    ("{{RSS_LIST}}" . ,(nori-site--render-list index item-layout list-items (find-file-noselect nori-site--contents-org-filename) t t))
	    ))
	 (page (nori-site--render-template template replacements)))
...
(defun nori-site--make-site ()
  "(Re)generate the full site, parsing the nori-site--entry-index and rendering all the pages."
  (interactive)
  ;; Parse the org-source
  (nori-site--build-entry-index nori-site--contents-org-filename)
  ...
  (let* ((publish-dir (expand-file-name nori-site--publish-directory))
	 (org-buffer (find-file-noselect nori-site--contents-org-filename))
    ...
    ;; Ensure publish directory exists, remove contents of old one if there was one
    ...
    (copy-directory nori-site--img-dir publish-dir t t nil) ; args are keep-time parents and copy-contents (only)
    ...
    ;; Render all pages from index
    (nori-site--render-all-pages org-buffer archive-html tag-cloud-html)
